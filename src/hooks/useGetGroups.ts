...
function useGetGroups(publicGroup : boolean) {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      if(publicGroup){
        const filteredGroupData = groupsData.filter((group) => group.admin !== user?.displayName && 
        !group.members?.includes(user?.displayName) && group.groupPrivacy !== 'Private');
        setGroups(filteredGroupData)
      }else{
        const ownGroup = groupsData.filter((group) => group.admin === user?.displayName ||
